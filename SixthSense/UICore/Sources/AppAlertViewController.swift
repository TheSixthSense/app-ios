...
public final class AppAlertViewController: UIViewController {
    ...
    func addButtons(action: AlertAction,
...
public extension UIViewController {
    ...
    func showAlert(
        ...
        return Observable.create { observer in
            let alertController = AppAlertViewController(alertTitle: title,
                                                         message: message)

            actions.forEach { action in
                alertController.addButtons(action: action) { action in
                    observer.onNext(action.style)
                    observer.onCompleted()
                }
            }
            self.present(alertController, animated: true, completion: nil)
            return Disposables.create { alertController.dismiss(animated: false) }
        }
    }
}
